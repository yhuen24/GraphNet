...
class GraphManager:
    ...
    def create_relationship(self, source_entity: str, source_type: str,
        ...
        Args:
            ...
            source_type: Source entity type (not used in query - matches by name only)
            target_entity: Target entity name
            target_type: Target entity type (not used in query - matches by name only)
        ...
        try:
            with self.driver.session() as session:
                # Match nodes by name only, regardless of label
                # This fixes the issue where entities are created with specific labels
                # (e.g., Person, Organization) but relationships try to match with "Entity"
                query = f"""
